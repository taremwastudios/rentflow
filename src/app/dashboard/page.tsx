  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="px-2">
        ...
        <p className="text-slate-500 dark:text-emerald-500/60 mt-1 font-medium text-sm transition-colors">Here&apos;s what&apos;s happening with your properties.</p>
      </div>
            ...
            <div className="flex-1">
              ...
              {(isPending || data.profile?.verificationStatus === "rejected") && (
                <Link
                  ...
                  className="inline-block mt-3 bg-amber-600 text-white px-5 py-2 rounded-lg text-xs font-bold transition-all active:scale-95"
                >
                  Upload Documents →
                </Link>
              )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <Link
            key={stat.label}
            href={stat.href}
            className="bg-white dark:bg-slate-950 rounded-xl p-5 border border-slate-100 dark:border-emerald-500/20 transition-all hover:border-emerald-500/40"
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl mb-3 transition-colors ${stat.color}`}>
              {stat.icon}
            </div>
            <div className="text-2xl font-black text-slate-900 dark:text-white transition-colors">{stat.value}</div>
            <div className="text-[10px] font-bold text-slate-400 dark:text-emerald-500/40 uppercase tracking-widest mt-0.5 transition-colors">{stat.label}</div>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-emerald-500/20 p-6 transition-colors shadow-sm dark:shadow-none">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white transition-colors">Recent Payments</h2>
            <Link href="/dashboard/payments" className="text-emerald-600 dark:text-emerald-400 text-xs font-bold hover:underline transition-colors">
              View all
            </Link>
          </div>
          {data.recentPayments.length === 0 ? (
            ...
            <div className="space-y-3">
              {data.recentPayments.map((payment: any) => (
                <div key={payment.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-emerald-500/5 transition-colors">
                  <div>
        ...
        {/* Quick Actions */}
        <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-emerald-500/20 p-6 transition-colors shadow-sm dark:shadow-none">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-6 transition-colors">Quick Actions</h2>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <Link
                key={action.label}
                href={action.href}
                className={`flex items-center gap-3 p-4 rounded-xl border border-transparent transition-all active:scale-95 ${action.color}`}
              >
